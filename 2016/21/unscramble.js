...
function rotateBased(line, current) {
    ...
    index = index || 1;
    const shift = (index % 2 === 1) ? 1 + Math.floor(index / 2) :
	  -3 + (index / 2);
    return current.slice(shift) + current.slice(0, shift);
}
...
function movePos(line, current) {
    let indexTo = parseInt(line[2], 10);
    let indexFrom = parseInt(line[5], 10);
    let next = current.split("");
